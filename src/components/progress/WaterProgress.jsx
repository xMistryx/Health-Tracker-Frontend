...
function generateFakeData(range) {
  const today = new Date();
  if (range === "today")
    return [
      { day: formatDate(today), oz: faker.number.int({ min: 40, max: 100 }) },
    ];
  if (range === "yesterday") {
    const yest = new Date(today);
    yest.setDate(today.getDate() - 1);
    return [
      { day: formatDate(yest), oz: faker.number.int({ min: 40, max: 100 }) },
    ];
  }
  if (range === "week")
    return Array.from({ length: 7 }, (_, i) => {
      ...
      return {
        day: formatDate(d),
        oz: faker.number.int({ min: 40, max: 100 }),
      };
    });
  if (range === "month")
...
export default function WaterProgress() {
  ...
  const data = generateFakeData(range);
  ...
  let summary = "";
  if (range === "today")
    summary = `Today (${data[0].day}) you drank ${data[0].oz} oz of water.`;
  else if (range === "yesterday")
    summary = `Yesterday (${data[0].day}) you drank ${data[0].oz} oz of water.`;
  else if (range === "week")
